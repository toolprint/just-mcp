...
use just_mcp::vector_search::{
    LibSqlVectorStore, MockEmbeddingProvider, OpenAIEmbeddingProvider, VectorSearchManager,
    ...
    match search_command {
        SearchCommands::Query {
            ...
            if mock_embeddings {
                ...
                return Err(anyhow::anyhow!("OpenAI API key required when not using mock embeddings. Set OPENAI_API_KEY environment variable or use --openai-api-key"));
            }
        }
        ...
        SearchCommands::Index {
            ...
            let manager = if mock_embeddings {
                create_search_manager_mock(&database).await?
            } else if let Some(_api_key) = openai_api_key {
                return Err(anyhow::anyhow!("OpenAI embedding provider not yet implemented in CLI"));
            } else {
                return Err(anyhow::anyhow!("OpenAI API key required when not using mock embeddings"));
            };
            ...
            println!("Indexing in {} batches of up to {} documents each", chunk_count, batch_size);
            ...
            for (chunk_idx, chunk) in documents.chunks(batch_size).enumerate() {
                ...
                let ids = manager.index_documents_chunked(chunk_docs, batch_size, "tasks").await?;
        ...
        SearchCommands::Stats { database } => {
            ...
            println!("Health status: {}", if healthy { "Healthy" } else { "Unhealthy" });
        ...
        SearchCommands::Filter { filter, limit, database } => {
            let manager = create_search_manager_mock(&database).await?;
            
            let filter_refs: Vec<(&str, &str)> = filter.iter().map(|(k, v)| (k.as_str(), v.as_str())).collect();
        ...
        SearchCommands::Text { text, limit, database } => {
            let manager = create_search_manager_mock(&database).await?;
            ...
            if results.is_empty() {
                ...
                println!("Found {} documents containing text '{}':", results.len(), text);
                println!();
                ...
                for (i, doc) in results.iter().enumerate() {
                    ...
                    println!("   Content: {}", highlighted.chars().take(200).collect::<String>());
...
async fn create_search_manager_mock(
    ...
    let vector_store = LibSqlVectorStore::new(database_path.to_string_lossy().to_string(), dimension);
...
async fn query_search<E: just_mcp::vector_search::EmbeddingProvider>(
    ...
    let results = manager.search_with_threshold(query, limit, threshold).await?;
    ...
    if results.is_empty() {
        ...
        for (i, result) in results.iter().enumerate() {
            ...
            println!("   Content: {}", result.document.content.chars().take(100).collect::<String>());
...
fn parse_justfile(justfile_path: &PathBuf) -> Result<Vec<Document>> {
    ...
    let mut documents = Vec::new();
    let justfile_name = justfile_path.file_name()
        .and_then(|n| n.to_str())
        .map(|s| s.to_string());
    ...
    for task in tasks {
        ...
        if !task.parameters.is_empty() {
            content_parts.push(format!("Parameters: {}", 
                task.parameters.iter()
                    .map(|p| p.name.clone())
                    .collect::<Vec<_>>()
                    .join(", ")
            ));
