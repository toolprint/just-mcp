...
impl<E: EmbeddingProvider, V: VectorStore> VectorSearchManager<E, V> {
    ...
    pub async fn index_tool_definitions_batch(
        &self,
        tool_definitions: Vec<Document>,
    ) -> Result<Vec<String>> {
        self.index_documents_batch(tool_definitions, "tool_definitions")
            .await
    }
    ...
    pub async fn index_documents_batch(
        ...
        let documents_with_embeddings: Vec<(Document, Vec<f32>)> =
    ...
    pub async fn index_documents_chunked(
        ...
        for (batch_idx, chunk) in documents.chunks(batch_size).enumerate() {
            ...
            let batch_ids = self
                .index_documents_batch(batch_docs, document_type)
                .await?;
    ...
    pub async fn advanced_search(
        ...
        let store = self.vector_store.lock().await;
        store
            .search_with_sql_filter(query_embedding, &sql_filters, limit, 0.0)
            .await
    }
    ...
    pub async fn update_document(&self, document_id: &str, document: Document) -> Result<()> {
        ...
        let mut store = self.vector_store.lock().await;
        store
            .update_document(document_id, document, embedding)
            .await
    }
    ...
    pub async fn hybrid_search(
        ...
        for result in text_results {
            if let Some(existing) = combined_results.get_mut(&result.document.id) {
                ...
                let entry =
                    SearchResult::new(result.document, weighted_score, 1.0 - weighted_score);
                combined_results.insert(entry.document.id.clone(), entry);
            }
        }
    ...
    pub async fn advanced_sql_search(
        ...
        let store = self.vector_store.lock().await;
        store
            .search_with_sql_filter(query_embedding, sql_conditions, limit, threshold)
            .await
    }
    ...
    pub async fn search_by_justfile(
        &self,
        justfile_name: &str,
        limit: usize,
    ) -> Result<Vec<Document>> {
        self.search_by_metadata(&[("justfile_name", justfile_name)], limit)
            .await
    }
    ...
    pub async fn search_by_task_pattern(
        &self,
        task_pattern: &str,
        limit: usize,
    ) -> Result<Vec<Document>> {
        ...
        let dummy_results = self
            .advanced_sql_search("dummy", &sql_conditions, limit, 0.0)
            .await?;
