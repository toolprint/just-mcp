...
pub trait VectorStore: Send + Sync {
    ...
    async fn update_document(&mut self, document_id: &str, document: Document, embedding: Vec<f32>) -> Result<()>;
    ...
    async fn add_documents_batch(&mut self, documents_with_embeddings: Vec<(Document, Vec<f32>)>) -> Result<Vec<String>>;
    ...
    async fn search_by_metadata(&self, metadata_filters: &[(&str, &str)], limit: usize) -> Result<Vec<Document>>;
...
impl LibSqlVectorStore {
    ...
    pub fn validate_embedding_dimension(embedding: &[f32], expected_dimension: usize) -> Result<()> {
    ...
    pub fn batch_cosine_similarity(query: &[f32], stored_embeddings: &[Vec<f32>]) -> Result<Vec<f32>> {
...
impl VectorStore for LibSqlVectorStore {
    ...
    async fn delete_document(&mut self, document_id: &str) -> Result<bool> {
        ...
        let mut stmt = conn.prepare("SELECT COUNT(*) FROM documents WHERE id = ?").await?;
    ...
    async fn add_documents_batch(&mut self, documents_with_embeddings: Vec<(Document, Vec<f32>)>) -> Result<Vec<String>> {
        ...
        if batch_success {
            ...
            tracing::debug!("Successfully committed batch of {} documents", document_ids.len());
        } else {
    ...
    async fn search_by_content(&self, text_query: &str, limit: usize) -> Result<Vec<Document>> {
        ...
        let mut rows = stmt.query(libsql::params![
            like_pattern,
            text_query,
            starts_with_pattern,
            limit as i64
