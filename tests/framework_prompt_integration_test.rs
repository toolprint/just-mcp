...
mod tests {
    ...
    async fn test_framework_server_prompt_integration() {
        ...
        assert!(prompts.contains(&"do-it".to_string()), "/just:do-it prompt not found");
    ...
    async fn test_do_it_prompt_execution_through_framework() {
        ...
        let result = prompt_provider.execute_prompt("do-it", Value::Object(arguments)).await;
        
        // Should succeed (even if no tasks match in mock provider)
        assert!(result.is_ok(), "Prompt execution failed: {:?}", result);
        
        let response = result.unwrap();
        assert!(response.contains("executed successfully"), "Unexpected response: {}", response);
    }
    
    #[tokio::test] 
    #[cfg(feature = "ultrafast-framework")]
    async fn test_framework_prompt_handler_implementation() {
        use ultrafast_mcp::{ListPromptsRequest, GetPromptRequest};
        ...
        let list_response = PromptHandler::list_prompts(prompt_provider.as_ref(), list_request).await;
        assert!(list_response.is_ok(), "ListPrompts failed: {:?}", list_response);
        ...
        assert!(do_it_prompt.description.as_ref().unwrap().contains("natural language"));
