...
mod framework_tests {
    ...
    async fn test_framework_resource_provider_creation() {
        ...
        assert!(result.is_ok(), "Should be able to create framework resource provider");
    ...
    async fn test_framework_resource_handler_trait() {
        use ultrafast_mcp::{ResourceHandler, ListResourcesRequest};
        ...
        assert!(response.resources.len() >= 0, "Should return a valid resource list");
    }
    ...
    async fn test_embedded_resources_available() {
        ...
        let handler: &dyn ResourceHandler = &provider;
        let request = ListResourcesRequest { cursor: None };
        let response = handler.list_resources(request).await.expect("Should list resources");
        
        // Check if we have embedded resources (like the best practices guide)
        let has_embedded_resources = response.resources.iter()
            .any(|r| r.uri.starts_with("file:///docs/guides/"));
        
        assert!(has_embedded_resources, "Should have embedded resources available");
    }

    #[tokio::test]
    async fn test_resource_read_functionality() {
        use ultrafast_mcp::{ResourceHandler, ListResourcesRequest, ReadResourceRequest};
        ...
        let list_response = handler.list_resources(list_request).await.expect("Should list resources");
        
        if let Some(resource) = list_response.resources.first() {
            // Try to read the first resource
            let read_request = ReadResourceRequest { uri: resource.uri.clone() };
            let read_result = handler.read_resource(read_request).await;
            
            assert!(read_result.is_ok(), "Should be able to read resource: {}", resource.uri);
