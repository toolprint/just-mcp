...
mod vector_store_tests {
    ...
    async fn test_add_and_get_document() -> Result<()> {
        ...
        // Add document
        let doc_id = store
            .add_document(document.clone(), embedding.clone())
            .await?;
    ...
    async fn test_document_update() -> Result<()> {
        ...
        store
            .update_document("update_test", updated_document, updated_embedding)
            .await?;
    ...
    async fn test_batch_operations() -> Result<()> {
        ...
        let batch_data = vec![
            (
                create_test_document("batch1", "First batch item"),
                create_test_embedding(384, 1.0),
            ),
            (
                create_test_document("batch2", "Second batch item"),
                create_test_embedding(384, 2.0),
            ),
            (
                create_test_document("batch3", "Third batch item"),
                create_test_embedding(384, 3.0),
            ),
        ];
    ...
    async fn test_embedding_dimension_validation() -> Result<()> {
        ...
        assert!(store
            .add_document(document.clone(), correct_embedding)
            .await
            .is_ok());
    ...
    async fn test_search_with_threshold() -> Result<()> {
        ...
        store
            .add_document(
                create_test_document("base", "Base document"),
                base_embedding.clone(),
            )
            .await?;
        store
            .add_document(
                create_test_document("similar", "Similar document"),
                similar_embedding,
            )
            .await?;
        store
            .add_document(
                create_test_document("different", "Different document"),
                different_embedding,
            )
            .await?;
