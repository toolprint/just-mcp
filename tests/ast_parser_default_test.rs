...
fn test_ast_parser_is_default() {
    ...
    assert!(parser.is_ast_parsing_available(), "AST parser should be available when feature is enabled");
...
"#;
    ...
    assert_eq!(metrics.ast_successes, 1, "AST parsing should have succeeded");
    assert_eq!(metrics.command_attempts, 0, "Should not have attempted command parsing");
    assert_eq!(metrics.regex_attempts, 0, "Should not have attempted regex parsing");
}
...
fn test_ast_parser_fallback_on_error() {
    ...
    let metrics = parser.get_metrics();
    assert!(metrics.ast_attempts > 0, "Should have attempted AST parsing");
    assert!(metrics.ast_successes == 0, "AST parsing should have failed");
    assert!(
        metrics.command_attempts > 0 || metrics.regex_attempts > 0 || metrics.minimal_task_creations > 0,
...
fn test_parser_works_without_ast_feature() {
    ...
    let parser = EnhancedJustfileParser::new().unwrap();
    assert!(!parser.is_ast_parsing_available(), "AST parser should not be available without feature");
...
fn test_parser_configuration_methods() {
    ...
    let metrics = parser.get_metrics();
    assert_eq!(metrics.ast_attempts, 0, "Should not attempt AST parsing when disabled");
    ...
    let metrics = parser.get_metrics();
    assert!(metrics.ast_attempts > 0, "Should attempt AST parsing when re-enabled");
}
...
fn test_diagnostics_includes_ast_info() {
    ...
    assert!(diagnostics.contains("AST:"), "Diagnostics should include AST metrics");
    assert!(diagnostics.contains("success rate"), "Should show success rates");
    assert!(diagnostics.contains("Preferred method:"), "Should indicate preferred parsing method");
}