...
mod integration_tests {
    ...
    use just_mcp::vector_search::{
        Document, LibSqlVectorStore, MockEmbeddingProvider, VectorSearchManager, VectorStore,
    };
    use std::collections::HashMap;
    use tempfile::TempDir;

    /// Create a test VectorSearchManager with mock embedding provider
    async fn create_test_manager() -> Result<(VectorSearchManager<MockEmbeddingProvider, LibSqlVectorStore>, TempDir)> {
    ...
    fn create_sample_document(id: &str, content: &str, task_name: &str, justfile: &str) -> Document {
        let mut metadata = HashMap::new();
        metadata.insert("type".to_string(), "justfile_task".to_string());
        metadata.insert("justfile_name".to_string(), justfile.to_string());
        metadata.insert("source_path".to_string(), format!("/test/{}/justfile", justfile));
    ...
    async fn test_tool_definitions_batch_indexing() -> Result<()> {
        ...
        let doc_ids = manager.index_tool_definitions_batch(tool_definitions).await?;
    ...
    async fn test_chunked_indexing() -> Result<()> {
        ...
        let doc_ids = manager.index_documents_chunked(tasks, batch_size, "chunked_tasks").await?;
    ...
    async fn test_basic_search() -> Result<()> {
        ...
        let build_tasks: Vec<_> = results.iter()
            .filter(|r| r.document.content.to_lowercase().contains("build") || 
    ...
    async fn test_advanced_search_with_filters() -> Result<()> {
        ...
        for result in &results {
            assert_eq!(result.document.justfile_name.as_ref().unwrap(), "backend_justfile");
        }
    ...
    async fn test_similar_tasks_search() -> Result<()> {
        ...
        let similar_tasks = manager.find_similar_tasks(
        ...
        for task in &similar_tasks {
            assert_ne!(task.document.content, "build: Compile the application with optimizations");
        }
    ...
    async fn test_advanced_sql_search() -> Result<()> {
        ...
        let results = manager.advanced_sql_search("testing", sql_conditions, 10, 0.0).await?;
    ...
    async fn test_search_by_justfile() -> Result<()> {
        ...
        let results = manager.search_by_justfile("deployment_justfile", 10).await?;
    ...
    async fn test_document_lifecycle() -> Result<()> {
        ...
        updated_document.content = "lifecycle: Updated test document lifecycle operations".to_string();
    ...
    async fn test_large_batch_operations() -> Result<()> {
        ...
        // Index in chunks
        let doc_ids = manager.index_documents_chunked(large_batch, 25, "large_batch").await?;
        
        assert_eq!(doc_ids.len(), 100);
        assert_eq!(manager.get_document_count().await?, 100);
        
        // Test search performance with large dataset
        let results = manager.search_documentation("automated operation", 10).await?;
        assert!(!results.is_empty());
    ...
    async fn test_embedding_provider_integration() -> Result<()> {
        ...
        assert_eq!(embedding_provider.model_name(), "mock-text-embedding-ada-002");
    ...
    async fn test_error_handling() -> Result<()> {
        ...
        assert!(manager.update_document("nonexistent", fake_doc).await.is_err());
