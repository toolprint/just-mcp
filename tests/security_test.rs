...
"#;
    ...
    assert!(result.stdout.contains("test; echo gotcha") || result.stdout.contains("'test; echo gotcha'"));
    ...
    assert!(!gotcha_on_own_line, "The 'echo gotcha' command should not have executed separately");
}
...
fn test_security_validator_direct() {
    ...
    assert!(validator.validate_parameter("name", "value; rm -rf /").is_err());
    ...
    assert_eq!(validator.sanitize_parameter("hello; rm -rf /"), "'hello; rm -rf /'");
}