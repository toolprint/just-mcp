...
use just_mcp::config_resource::{ConfigDataCollector, ConfigResourceProvider, CombinedResourceProvider};
...
async fn create_test_handler_with_config() -> MessageHandler {
    ...
    let combined_provider = Arc::new(CombinedResourceProvider::new(embedded_provider, config_provider));
...
async fn test_resources_list_includes_config_json() {
    ...
    let has_config = resources.iter().any(|resource| {
        resource.get("uri").unwrap().as_str().unwrap() == "file:///config.json"
    });
    assert!(has_config, "Resources list should include file:///config.json");
    
    // Verify config.json resource structure
    let config_resource = resources.iter()
    ...
    assert_eq!(config_resource.get("title").unwrap(), "just-mcp Configuration");
...
async fn test_config_json_read_invalid_uri() {
    ...
    assert!(result.is_err(), "Should return an error for invalid config URI");
...
async fn test_combined_provider_serves_both_embedded_and_config() {
    ...
    let has_config = resources.iter().any(|r| {
        r.get("uri").unwrap().as_str().unwrap() == "file:///config.json"
    });
    assert!(has_config);
    
    // Should have embedded content
    let has_embedded = resources.iter().any(|r| {
        r.get("uri").unwrap().as_str().unwrap().starts_with("file:///docs/guides/")
    });
    assert!(has_embedded);
